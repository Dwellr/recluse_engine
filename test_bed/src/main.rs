...
use hal::{buffer, command, format as f, format::{AsFormat, ChannelType, Rgba8Srgb as ColorFormat, Swizzle}, image as i, memory as m, pass, pass::Subpass, pool, prelude::*, pso, pso::{PipelineStage, ShaderStageFlags, VertexInputRate}, queue::{QueueGroup, Submission}, window, Instance};
...
const COLOR_NAME: i::SubresourceRange = i::SubresourceRange {
...
fn main() {
    ...
    #[cfg(not(target_arch = "wasm32"))]
    let event_loop: EventLoop<()> = winit::event_loop::EventLoop::new();
    ...
    let wb = winit::window::WindowBuilder::new()
        ...
        .with_title("Testbed".to_string());
    // Instantiate backend
    #[cfg(not(feature = "gl"))]
    let (_window, _instance, mut adapters, surface) = {
        let window = wb.build(&event_loop).unwrap();
        let instance = back::Instance::create("gfx-rs testbed", 1);
        ...
        match event {
            winit::event::WindowEvent { event, .. } => match event {
                ...
                winit::event::WindowEvent::Resized(dims) => {
                    ...
                    renderer.dimensions = window::Event2D {
