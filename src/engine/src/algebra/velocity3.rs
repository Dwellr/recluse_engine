...
impl<N: RealField> Velocity3<N> {
    ...
    pub fn linear(vx: N, vy: N, vz: N) -> Self {
        Self::new(Vector3::new(vx, vy, vz), na::zero())
    }

    #[inline]
    pub fn angular(wx: N, wy: N, wz: N) -> Self {
        Self::new(na::zero(), Vector3::new(wx, wy, wz))
    }

    pub fn between_pos(start: &Isometry3<N>, end: Isometry3<N>, time: N) -> Self {
        let delta = end / start;
    ...
    #[inline]
    pub fn angular_vec(&self) -> Vector3<N> {
        self.angular
    }
    ...
    pub fn as_vector_mut(&mut self) -> &mut Vectort6<N> {
        unsafe {
            mem::transmute(self)
        }
    }
